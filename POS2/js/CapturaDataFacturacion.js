$('document').ready(function($){
  ...
  FKTicket:{
    ...
    function submitForm() {	
        ...
        $.ajax({				
            type : 'POST',
            url: "Consultas/DatosDeFacturas.php",
            data: $('#DatosParaFacturacion').serialize(),
            cache: false,
            beforeSend: function(){	
      ...
      success: function(dataResult){
        ...
        setTimeout(function(){ 
            $("#submit_registro").html("Guardar <i class='fas fa-save'></i>");
             ...
             $("#DatosParaFacturacion")[0].reset();
             $("#CapturaFacturacion").removeClass("in");
             $(".modal-backdrop").remove();
             $("#CapturaFacturacion").hide();
           

             $('#ExitoFact').modal('toggle'); 
             setTimeout(function(){ 
                 $('#ExitoFact').modal('hide') 
             }, 2000); // abrir
