...
<head>
  ...
  <title>Almacén | <?echo $row['ID_H_O_D']?> <?echo $row['Nombre_Sucursal']?> </title>
...
<div class="card text-center">
  ...
  <div >
  <button type="button"  style="<?if($row['Fk_Sucursal'] == 21){
   
   echo "display:true;";
 
} else {
  echo "display:none;";
}?>" class="btn btn-success" data-toggle="modal" data-target="#IniciaCapturaDeFacturaProceso" class="btn btn-default">
  Capturar nueva factura <i class="fas fa-file-invoice"></i>
</button>
</div>

</div>
    
<input type="text" name="" id="xd">

<script>

//Cuando se presione enter
$('#xd').keypress(function (e) {
    if(e.which == 40){
        alert('ok');
    }
});
