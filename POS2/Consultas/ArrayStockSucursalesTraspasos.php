...
Stock_POS.Fk_Sucursal = SucursalesCorre.ID_SucursalC AND Stock_POS.Tipo_Servicio= Servicios_POS.Servicio_ID AND Productos_POS.ID_Prod_POS =Stock_POS.ID_Prod_POS AND
 Stock_POS.ID_H_O_D ='".$row['ID_H_O_D']."' AND Stock_POS.Fk_Sucursal=21";
...
while($fila=$result->fetch_assoc()){
    ...
    $data[$c]["Acciones"] = ["<a href=https://controlfarmacia.com/POS2/GeneradorTraspasos?idProd=".base64_encode($fila["Folio_Prod_Stock"])."  type='button' class='btn btn-success  btn-sm '><i class='fas fa-exchange-alt'></i></a> "];
