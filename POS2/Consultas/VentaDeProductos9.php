...
if ($conn)
{
	...
	while ($row = mysqli_fetch_array($fetch)) {
        ...
        $row_array['fkid9']=$row['ID_Prod_POS'];
        $row_array['clavad9']=$row['Clave_adicional'];
            $row_array['codbarras9']=$row['Cod_Barra'];
            $row_array['nombreprod9']=$row['Nombre_Prod'];
            $row_array['precioprod9']=$row['Precio_Venta'];
            $row_array['lote9']=$row['Lote'];
            $row_array['identificadortip9']=$row['Tipo_Servicio'];
