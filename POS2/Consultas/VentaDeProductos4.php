...
if ($conn)
{
	...
	while ($row = mysqli_fetch_array($fetch)) {
        ...
        $row_array['fkid4']=$row['ID_Prod_POS'];
        $row_array['clavad4']=$row['Clave_adicional'];
            $row_array['codbarras4']=$row['Cod_Barra'];
            $row_array['nombreprod4']=$row['Nombre_Prod'];
            $row_array['precioprod4']=$row['Precio_Venta'];
            $row_array['lote4']=$row['Lote'];
            $row_array['identificadortip4']=$row['Tipo_Servicio'];
