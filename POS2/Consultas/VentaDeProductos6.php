...
if ($conn)
{
	...
	while ($row = mysqli_fetch_array($fetch)) {
        ...
        $row_array['fkid6']=$row['ID_Prod_POS'];
        $row_array['clavad6']=$row['Clave_adicional'];
            $row_array['codbarras6']=$row['Cod_Barra'];
            $row_array['nombreprod6']=$row['Nombre_Prod'];
            $row_array['precioprod6']=$row['Precio_Venta'];
            $row_array['lote6']=$row['Lote'];
            $row_array['identificadortip6']=$row['Tipo_Servicio'];
