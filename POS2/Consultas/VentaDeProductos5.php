...
if ($conn)
{
	...
	while ($row = mysqli_fetch_array($fetch)) {
        ...
        $row_array['fkid5']=$row['ID_Prod_POS'];
        $row_array['clavad5']=$row['Clave_adicional'];
            $row_array['codbarras5']=$row['Cod_Barra'];
            $row_array['nombreprod5']=$row['Nombre_Prod'];
            $row_array['precioprod5']=$row['Precio_Venta'];
            $row_array['lote5']=$row['Lote'];
            $row_array['identificadortip5']=$row['Tipo_Servicio'];
