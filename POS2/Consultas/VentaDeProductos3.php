...
if ($conn)
{
	...
	while ($row = mysqli_fetch_array($fetch)) {
        ...
        $row_array['fkid3']=$row['ID_Prod_POS'];
        $row_array['clavad3']=$row['Clave_adicional'];
            $row_array['codbarras3']=$row['Cod_Barra'];
            $row_array['nombreprod3']=$row['Nombre_Prod'];
            $row_array['precioprod3']=$row['Precio_Venta'];
            $row_array['lote3']=$row['Lote'];
            $row_array['identificadortip3']=$row['Tipo_Servicio'];
