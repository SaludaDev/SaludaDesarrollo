...
if ($conn)
{
	...
	while ($row = mysqli_fetch_array($fetch)) {
        ...
        $row_array['fkid7']=$row['ID_Prod_POS'];
        $row_array['clavad7']=$row['Clave_adicional'];
            $row_array['codbarras7']=$row['Cod_Barra'];
            $row_array['nombreprod7']=$row['Nombre_Prod'];
            $row_array['precioprod7']=$row['Precio_Venta'];
            $row_array['lote7']=$row['Lote'];
            $row_array['identificadortip7']=$row['Tipo_Servicio'];
