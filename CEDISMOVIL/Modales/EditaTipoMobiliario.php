...
$user_id=null;
$sql1= "SELECT * FROM `Tipos_Mobiliarios_POS` WHERE ID_H_O_D ='".$row['ID_H_O_D']."' AND Tip_Mob_ID = ".$_POST["id"];
...
<form action="javascript:void(0)" method="post" id="ActualizaTipoMobiliarios" >
<div class="form-group">
    <label for="exampleFormControlInput1">Folio</label>
    <div class="input-group mb-3">
  <div class="input-group-prepend">  <span class="input-group-text" id="Tarjeta"><i class="fas fa-receipt"></i></span>
  </div>
  <input type="text" class="form-control " disabled readonly value="<? echo $Especialistas->Tip_Mob_ID; ?>">
    </div>
    </div>
  ...
  <input type="text" class="form-control "  id="actnomttpmb" name="ActNomTTPMB" value="<? echo $Especialistas->Nom_Tip_Mob; ?>" aria-describedby="basic-addon1" maxlength="60">            
</div></div></div>
...
<div class="row">
  ...
  <select name="ActVigenciaCatTPMB" class="form-control" id="actualizavigenciacatTTPMB" onchange="ActualizaTipoProdVigencia();">
...
<button id="VigenciaBDTipoProd" class="btn btn-default btn-sm" style=<? echo $Especialistas->Cod_Estado; ?>><? echo $Especialistas->Estado; ?></button> 
    ...
    <input type="text"  class="form-control " hidden readonly name="ActVigEstTTPMB" id="vigenciacatactTTP">
    <input type="text" class="form-control " hidden  readonly id="actusuariottp" name="ActUsuarioTTPMB" readonly value="<?echo $row['Nombre_Apellidos']?>">
<input type="text" class="form-control "  hidden  readonly id="actsistemattp" name="ActSistemaTTPMB" readonly value="POS <?echo $row['Nombre_rol']?>">
<input type="hidden" name="Id_TTPMB" id="id" value="<?php echo $Especialistas->Tip_Mob_ID; ?>">
<button type="submit"  id="submit"  class="btn btn-info">Aplicar cambios <i class="fas fa-check"></i></button>
                          
</form>
<script src="js/ActualizaTTPMB.js"></script>
...
var combo = document.getElementById("actualizavigenciacatTTPMB");
