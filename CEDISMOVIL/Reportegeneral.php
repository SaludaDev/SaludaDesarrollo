...
$TipoReporte="Desglose General de ventas";
$fcha = date("Y-m-d");
if (PHP_SAPI == 'cli')
...
$objPHPExcel->getProperties()->setCreator("Doctor Consulta")
							 ->setLastModifiedBy("")
							 ->setTitle("Reporte general de ventas doctor consulta")
							 ->setSubject("Reporte general de ventas doctor consulta")
							 ->setDescription("Reporte general de ventas doctor consulta")
							 ->setKeywords("Ventas Doctor Consulta")
							 ->setCategory("Reporte general de ventas doctor consulta");
...
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('B2', 'Documento: General')
			->setCellValue('C2', 'Usuario: Todos')
              ->setCellValue('D2', 'Generado:')
              ->setCellValue('E2', $fcha)
...
/*Extraer datos de MYSQL*/
   ...
   require 'Consultas/db_connection.php';
    ...
    Fk_sucursal,Ventas_POS.AgregadoPor,Ventas_POS.AgregadoEl, Ventas_POS.Lote,Ventas_POS.ID_H_O_D,SucursalesCorre.ID_SucursalC,SucursalesCorre.Nombre_Sucursal
     ...
     $sql2="SELECT SUM(Total_VentaG) as TotalDeventa FROM Ventas_POS ";
     $sql3="SELECT FormaDePago, SUM(Total_VentaG) as TotalEfectivo FROM Ventas_POS WHERE FormaDePago ='Efectivo' ";
     $sql4="SELECT IFNULL(Ventas_POS.FormaDePago,'Cheque')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalCheque FROM Ventas_POS WHERE FormaDePago ='Cheque' ";
     $sql5="SELECT IFNULL(Ventas_POS.FormaDePago,'Tarjeta')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalTarjeta FROM Ventas_POS WHERE FormaDePago ='Tarjeta'";
     $sql6="SELECT IFNULL(Ventas_POS.FormaDePago,'Credito')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalCredito FROM Ventas_POS WHERE FormaDePago ='Credito'";
     $sql7="SELECT IFNULL(Ventas_POS.FormaDePago,'Vale')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalVale FROM Ventas_POS WHERE FormaDePago ='Vale'";
     $sql8="SELECT IFNULL(Ventas_POS.FormaDePago,'Transferencia')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalTransferencia FROM Ventas_POS WHERE FormaDePago ='Transferencia'";
...
header('Content-Disposition: attachment;filename='.basename("Reporte ventas general doctor consulta $fcha.xls"));
