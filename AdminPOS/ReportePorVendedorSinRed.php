...
$fcha = date("Y-m-d");
$Creador=$_POST["UsuarioPorSucursaloff"];
$VendedorVentas=$_POST["VendedorPorVendedoroff"];

$FechaIni=$_POST["FechaInicialoff"];
$FechaFin=$_POST["FechaFinaloff"];
$TipoReporte="Reporte ventas de $VendedorVentas Del $FechaIni al $FechaFin";
...
$objPHPExcel->getProperties()->setCreator($Creador)
							 ...
							 ->setTitle("Reporte ventas de $VendedorVentas Del $FechaIni al $FechaFin")
							 ->setSubject($Empresaa)
							 ->setDescription("Reporte ventas de $VendedorVentas Del $FechaIni al $FechaFin")
							 ->setKeywords("Reporte ventas de $VendedorVentas Del $FechaIni al $FechaFin")
							 ->setCategory("Reporte ventas de $VendedorVentas Del $FechaIni al $FechaFin");
...
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('B2', "Reporte ventas de ")
			->setCellValue('C2', $VendedorVentas)
              ->setCellValue('D2', 'Periodo:')
              ->setCellValue('E2', $FechaIni )
              ->setCellValue('F2', 'Al' )
              ->setCellValue('G2', $FechaFin )
              ->setCellValue('H2', $FechaFin )
...
/*Extraer datos de MYSQL*/
   ...
   require 'Consultas/db_connection.php';
    ...
    Fk_sucursal,Ventas_POS.AgregadoPor,Ventas_POS.AgregadoEl, Ventas_POS.Lote,Ventas_POS.ID_H_O_D,SucursalesCorre.ID_SucursalC,SucursalesCorre.Nombre_Sucursal
     FROM Ventas_POS,SucursalesCorre,Servicios_POS where  Ventas_POS.Fk_sucursal= SucursalesCorre.ID_SucursalC AND Ventas_POS.Identificador_tipo=Servicios_POS.Servicio_ID AND
     Ventas_POS.AgregadoPor='$VendedorVentas' AND Ventas_POS.Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin' ";
     $sql2="SELECT SUM(Total_VentaG) as TotalDeventa FROM Ventas_POS WHERE AgregadoPor='$VendedorVentas' AND Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin' ";
     $sql3="SELECT FormaDePago, SUM(Total_VentaG) as TotalEfectivo FROM Ventas_POS WHERE FormaDePago ='Efectivo' AND AgregadoPor='$VendedorVentas' AND Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin'";
     $sql4="SELECT IFNULL(Ventas_POS.FormaDePago,'Cheque')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalCheque FROM Ventas_POS WHERE FormaDePago ='Cheque' AND AgregadoPor='$VendedorVentas' AND Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin' ";
     $sql5="SELECT IFNULL(Ventas_POS.FormaDePago,'Tarjeta')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalTarjeta FROM Ventas_POS WHERE FormaDePago ='Tarjeta' AND AgregadoPor='$VendedorVentas' AND Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin' ";
     $sql6="SELECT IFNULL(Ventas_POS.FormaDePago,'Credito')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalCredito FROM Ventas_POS WHERE FormaDePago ='Credito' AND AgregadoPor='$VendedorVentas' AND Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin'";
     $sql7="SELECT IFNULL(Ventas_POS.FormaDePago,'Vale')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalVale FROM Ventas_POS WHERE FormaDePago ='Vale' AND AgregadoPor='$VendedorVentas' AND Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin' ";
     $sql8="SELECT IFNULL(Ventas_POS.FormaDePago,'Transferencia')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalTransferencia FROM Ventas_POS WHERE FormaDePago ='Transferencia' AND AgregadoPor='$VendedorVentas' AND Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin'";
...
header('Content-Disposition: attachment;filename='.basename("Reporte ventas de $VendedorVentas Del $FechaIni al $FechaFin.xls"));
