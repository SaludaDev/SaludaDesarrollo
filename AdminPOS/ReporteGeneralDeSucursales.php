...
$fcha = date("Y-m-d");
$Creador=$_POST["UsuarioMaestro"];

$FechaIni=$_POST["FechaInicialGeneral"];
$FechaFin=$_POST["FechaFinalGeneral"];
$TipoReporte="Reporte ventas  Del $FechaIni al $FechaFin";
...
$objPHPExcel->getProperties()->setCreator($Creador)
							 ...
							 ->setTitle("Reporte ventas Del $FechaIni al $FechaFin")
							 ->setSubject($Empresaa)
							 ->setDescription("Reporte ventas Del $FechaIni al $FechaFin")
							 ->setKeywords("Reporte ventas Del $FechaIni al $FechaFin")
							 ->setCategory("Reporte ventas  Del $FechaIni al $FechaFin");
...
$objPHPExcel->getActiveSheet()->getStyle('A5:M5')->applyFromArray($styleArray4);		
...
/*Extraer datos de MYSQL*/
   ...
   require 'Consultas/db_connection.php';
    ...
    Fk_sucursal,Ventas_POS.AgregadoPor,Ventas_POS.AgregadoEl, Ventas_POS.Lote,Ventas_POS.ID_H_O_D,SucursalesCorre.ID_SucursalC,SucursalesCorre.Nombre_Sucursal
     FROM Ventas_POS,SucursalesCorre,Servicios_POS where 
     ...
     $sql2="SELECT SUM(Total_VentaG) as TotalDeventa FROM Ventas_POS WHERE Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin' ";
     ...
     $sql4="SELECT IFNULL(Ventas_POS.FormaDePago,'Cheque')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalCheque FROM Ventas_POS WHERE FormaDePago ='Cheque' AND  Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin' ";
     $sql5="SELECT IFNULL(Ventas_POS.FormaDePago,'Tarjeta')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalTarjeta FROM Ventas_POS WHERE FormaDePago ='Tarjeta' AND  Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin' ";
     $sql6="SELECT IFNULL(Ventas_POS.FormaDePago,'Credito')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalCredito FROM Ventas_POS WHERE FormaDePago ='Credito' AND Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin'";
     $sql7="SELECT IFNULL(Ventas_POS.FormaDePago,'Vale')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalVale FROM Ventas_POS WHERE FormaDePago ='Vale' AND  Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin' ";
     $sql8="SELECT IFNULL(Ventas_POS.FormaDePago,'Transferencia')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalTransferencia FROM Ventas_POS WHERE FormaDePago ='Transferencia'  AND Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin'";
