...
$sql = "SELECT Cod_Barra,Nombre_Prod,Precio_Venta,Precio_C,Max_Existencia,Min_Existencia,Tipo,Tipo_Servicio,FkCategoria,FkMarca,FkPresentacion,ID_H_O_D, Proveedor2,Proveedor1,
   Estatus,Sistema,AgregadoPor,Lote_Med,Fecha_Caducidad,Stock FROM Productos_POS 
...
//include database configuration file
if ($row['Cod_Barra'] == $Cod_Barra and $row['Nombre_Prod'] == $Nombre_Prod  and  $row['Precio_Venta'] == $Precio_Venta and  $row['Precio_C'] == $Precio_C
    ...
    `Lote_Med`,`Fecha_Caducidad`,`Stock`,`Vendido`,`Saldo`,`Tipo_Servicio`,`Componente_Activo`,`Tipo`,`FkCategoria`,`FkMarca`,`FkPresentacion`, `Proveedor1`,`Proveedor2`,`RecetaMedica`,`Estatus`,`CodigoEstatus`,`Sistema`,`AgregadoPor`,`ID_H_O_D`) 
