...
Creditos_POS.Cant_Apertura,Creditos_POS.Costo_Tratamiento,Creditos_POS.Fk_Sucursal,
...
<?php if($query->num_rows>0):?>
  <div class="text-center">
	...
	<table  id="CreditosFinalizados" class="table table-hover">
<thead>
    ...
    <th>Promocion/Descuento promocion</th>
    <th>Costo total</th>
    <th>Saldo</th>
    <th>Estatus</th>
    <th>Acciones</th>
...
<tr>
  ...
  <td ><?php echo $Categorias["Promocion"]; ?><br> $<?php echo $Categorias["Costo_Descuento"]; ?> 
  ...
  <td><button style="<?php if($Categorias["Saldo"] == 0.00){
   echo "background-color:#2BBB1D !important";
} else {
  echo "background-color:#ffc107 !important";
}?>"class="btn btn-default btn-sm" ><?php if($Categorias["Saldo"] == 0.00){
  echo "Finalizado";
} else {
...
<a data-id="<?php echo  $Categorias["Folio_Credito"];?>" class="btn-edit dropdown-item" >Imprimir comprobante <i class="fas fa-print"></i></a>
<a data-id="<?php echo $Categorias["Folio_Credito"];?>" class="btn-edit2 dropdown-item" >Generar PDF <i class="fas fa-file-pdf"></i></a>
...
<?php else:?>
	<p class="alert alert-warning">No hay créditos finalizados</p>
<?php endif;?>
