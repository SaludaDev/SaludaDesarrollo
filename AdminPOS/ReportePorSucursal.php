...
$fcha = date("Y-m-d");
$Creador=$_POST["UsuarioPorSucursal"];
$SucursalVentas=$_POST["SucursalPorSucursal"];
$SucursalConsulta=$_POST["Sucursal"];
...
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('C2', 'Usuario: Todos')
              ...
              ->setCellValue('F5', 'FORMA DE PAGO')
              ->setCellValue('G5', 'LOTE')
              ->setCellValue('H5', 'PZAS VENDIDAS')
              ->setCellValue('I5', 'P.U')
              ->setCellValue('J5', 'DESCUENTO')
              ->setCellValue('K5', 'IMPORTE')
              ->setCellValue('L5', 'FECHA Y HORA DE VENTA')
              ->setCellValue('M5', 'VENDEDOR')
              ->setCellValue('N5', 'CLIENTE')
              ->setCellValue('O5', 'TOTAL VENTA');
...
/*Extraer datos de MYSQL*/
   ...
   require 'Consultas/db_connection.php';
	...
	$sql="SELECT Ventas_POS.Folio_Ticket,Ventas_POS.Fk_Caja,Ventas_POS.Venta_POS_ID,Ventas_POS.Identificador_tipo,Ventas_POS.Cod_Barra,Ventas_POS.FormaDePago,
    Ventas_POS.DescuentoAplicado,Ventas_POS.Cliente,
    ...
    Fk_sucursal,Ventas_POS.AgregadoPor,Ventas_POS.AgregadoEl, Ventas_POS.Lote,Ventas_POS.ID_H_O_D,SucursalesCorre.ID_SucursalC,SucursalesCorre.Nombre_Sucursal
     FROM Ventas_POS,SucursalesCorre,Servicios_POS where Ventas_POS.Fk_sucursal='$SucursalConsulta'  AND
      Ventas_POS.Fk_sucursal= SucursalesCorre.ID_SucursalC AND Ventas_POS.Identificador_tipo=Servicios_POS.Servicio_ID AND Ventas_POS.Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin' ";


     $sql2="SELECT SUM(Total_VentaG) as TotalDeventa FROM Ventas_POS WHERE Fk_sucursal='$SucursalConsulta' AND Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin' ";
     $sql3="SELECT FormaDePago, SUM(Total_VentaG) as TotalEfectivo FROM Ventas_POS WHERE FormaDePago ='Efectivo' AND Fk_sucursal='$SucursalConsulta' AND Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin'";
     $sql4="SELECT IFNULL(Ventas_POS.FormaDePago,'Cheque')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalCheque FROM Ventas_POS WHERE FormaDePago ='Cheque' AND Fk_sucursal='$SucursalConsulta' AND Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin' ";
     $sql5="SELECT IFNULL(Ventas_POS.FormaDePago,'Tarjeta')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalTarjeta FROM Ventas_POS WHERE FormaDePago ='Tarjeta' AND Fk_sucursal='$SucursalConsulta' AND Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin' ";
     $sql6="SELECT IFNULL(Ventas_POS.FormaDePago,'Credito')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalCredito FROM Ventas_POS WHERE FormaDePago ='Credito' AND Fk_sucursal='$SucursalConsulta' AND Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin'";
     $sql7="SELECT IFNULL(Ventas_POS.FormaDePago,'Vale')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalVale FROM Ventas_POS WHERE FormaDePago ='Vale' AND Fk_sucursal='$SucursalConsulta' AND Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin' ";
     $sql8="SELECT IFNULL(Ventas_POS.FormaDePago,'Transferencia')FormaDePago, COALESCE(SUM(Total_VentaG), 0.00) as TotalTransferencia FROM Ventas_POS WHERE FormaDePago ='Transferencia' AND Fk_sucursal='$SucursalConsulta' AND Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin'";
     $sql9="SELECT IFNULL(Ventas_POS.FormaDePago,'Crédito Enfermería')FormaDePago, COALESCE(SUM(Importe), 0.00) as TotalCréditoEnfermería FROM Ventas_POS WHERE FormaDePago ='Crédito Enfermería' AND Fk_sucursal='$SucursalConsulta' AND Fecha_venta BETWEEN '$FechaIni' AND '$FechaFin'";
	...
	while ($row=mysqli_fetch_array($query)){
        ...
        $Total_VentaG=$row['Total_VentaG'];
            ...
            ->setCellValue($f, $FormaDePago)
            ->setCellValue($g, $Lote)
			->setCellValue($h, $Cantidad_Venta)
            ->setCellValue($i, $Total_Venta)
			->setCellValue($j,$DescuentoAplicado )
            ->setCellValue($k,$Importe )
            ->setCellValue($l, $AgregadoEl)
            ->setCellValue($m, $AgregadoPor)
            ->setCellValue($n, $Cliente)
            ->setCellValue($o, $Total_VentaG);
            $cel++;
                                    ...
                                    while ($row7=mysqli_fetch_array($query7)){
                                        ...
                                        $TotalVale=$row7['TotalVale'];
                                            ...
                                            while ($row9=mysqli_fetch_array($query9)){
                                                ...
                                                $FormaDePago=$row9['FormaDePago'];
                                                $TotalCréditoEnfermería=$row9['TotalCréditoEnfermería'];
                                            }
                                                    
                                          
                                                    $objPHPExcel->getActiveSheet() ->setCellValue("I$cel", "$FormaDePago");
                                                    $objPHPExcel->getActiveSheet() ->setCellValue("J$cel", "$TotalCréditoEnfermería");
