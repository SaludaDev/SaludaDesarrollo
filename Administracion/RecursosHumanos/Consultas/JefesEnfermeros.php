...
<a data-id="<?php echo $PersonalEnfermeria["Enfermero_ID"];?>" class="btn-edit2 dropdown-item" >Editar datos <i class="fas fa-pencil-alt"></i></a>
  ...
  echo "display:block;";
}?> class="dropdown-item"  href="http://localhost:8080/RecursosHumanos/index.php?nombrerh=<?echo $row['Nombre_Apellidos']?>&&rol=<?echo $row['Nombre_rol']?>&&file=<?echo $row['file_name']?>&&
  idempleado=<?php echo $PersonalEnfermeria["Enfermero_ID"]; ?>&&nombreempleado=<?php echo $PersonalEnfermeria["Nombre_Apellidos"]; ?>&&sucursalnombre=<?php echo $PersonalEnfermeria["Nombre_Sucursal"]; ?>&&puesto=Enfermería">Capturar datos biometricos <i class="fas fa-fingerprint"></i></a>
...
<?php endif;?>
  ...
  <script>
    ...
    $(".btn-HistorialEmpleados").click(function(){
  		id = $(this).data("id");
  		$.post("https://www.somosgrupoe.com/Administracion/RecursosHumanos/Modales/HistorialEnfermeros.php","id="+id,function(data){
              $("#form-edit").html(data);
    ...
    $(".btn-baja").click(function(){
  		id = $(this).data("id");
  		$.post("https://www.somosgrupoe.com/Administracion/RecursosHumanos/Modales/BajaEnfermero.php","id="+id,function(data){
              $("#form-edit").html(data);
              $("#Titulo").html("Historial datos de empleado");
              $("#Di").removeClass("modal-dialog modal-lg modal-notify modal-info");
              $("#Di").removeClass("modal-dialog modal-lg modal-notify modal-danger");
