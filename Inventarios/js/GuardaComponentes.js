$('document').ready(function ($) {
    ...
    function submitForm() {
      ...
      $.ajax({
        type: 'POST',
        url: "Consultas/AgregaComponentes.php",
        data: $('#Agregandocomponentes').serialize(),
        cache: false,
        beforeSend: function () {
        ...
        success: function (dataResult) {
          ...
          if (dataResult.statusCode == 250) {
            ...
            $("#Agregandocomponentes")[0].reset();
            $("#AltaProductos").removeClass("in");
            $(".modal-backdrop").remove();
            $("#AltaProductos").hide();
          
  
            $('#AltaNuevoComponente').modal('toggle');
            setTimeout(function () {
              $('#AltaNuevoComponente').modal('hide')
            }, 4000); // abrir
            CargaComponentesActivos();
  
            //  Solucionar muestra de modal de AltaNuevoComponente
