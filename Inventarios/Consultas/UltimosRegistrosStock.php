...
while($fila=$result->fetch_assoc()){
    ...
    $data[$c]["Acciones"] = ["<button class='btn btn-primary btn-sm dropdown-toggle' type='button' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'><i class='fas fa-th-list fa-1x'></i></button><div class='dropdown-menu'><a href=https://saludaclinicas.com/AdminPOS/AsignacionSucursalesStock?idProd=".$fila["ID_Prod_POS"]." class='btn-edit  dropdown-item' >Asignar en mas sucursales <i class='fas fa-paste'></i></a><a data-id='".$fila["ID_Prod_POS"]."' class='btn-VerDistribucion  dropdown-item' >Ver distribucion <i class='fas fa-hospital-alt'></i></a><a data-id='".$fila["ID_Prod_POS"]."' class='btn-editProd dropdown-item' >Editar datos <i class='fas fa-pencil-alt'></i></a><a data-id='".$fila["ID_Prod_POS"]."' class='btn-History dropdown-item' >Ver movimientos <i class='fas fa-history'></i></a><a data-id='".$fila["ID_Prod_POS"]."' class='btn-Delete dropdown-item' >Eliminar Producto <i class='fas fa-minus-circle'></i></a></div> "];
