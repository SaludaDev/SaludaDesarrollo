...
$ColorEstatus=  $conn -> real_escape_string(htmlentities(strip_tags(Trim($_POST['vigencia']))));
    ...
    $sql = "SELECT Nombre_Apellidos,Fk_Usuario,Correo_Electronico,ID_H_O_D FROM Personal_Agenda WHERE Nombre_Apellidos='$Nombre_Apellidos' AND ID_H_O_D='$ID_H_O_D'
    AND Fk_Usuario='$Fk_Usuario'AND Correo_Electronico='$Correo_Electronico'";
    ...
    $row = mysqli_fetch_assoc($resultset);	
        ...
        if($row['Nombre_Apellidos']==$Nombre_Apellidos AND $row['ID_H_O_D']==$ID_H_O_D AND $row['Fk_Usuario']==$Fk_Usuario AND $row['Correo_Electronico']==$Correo_Electronico){				
            ...
            $sql = "INSERT INTO `Personal_Agenda`( `Nombre_Apellidos`,`Password`,`file_name`,`Fk_Usuario`,`Fecha_Nacimiento`,`Correo_Electronico`,`Telefono`,`AgregadoPor`,`ID_H_O_D`,`Estatus`,`ColorEstatus`) 
 VALUES ('$Nombre_Apellidos','$Password','$uploadedFile','$Fk_Usuario','$Fecha_Nacimiento','$Correo_Electronico','$Telefono','$AgregadoPor','$ID_H_O_D','$Estatus','$ColorEstatus')";
