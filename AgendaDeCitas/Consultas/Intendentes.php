...
<a data-id="<?php echo $Especialidades["Intendencia_ID"];?>" class="btn-edit2 dropdown-item" >Editar datos <i class="fas fa-pencil-alt"></i></a>
  ...
  class="dropdown-item"  href="http://localhost/AdminPOS/index.php?nombrerh=<?php echo $row['Nombre_Apellidos']?>&&rol=<?php echo $row['Nombre_rol']?>&&file=<?echo $row['file_name']?>&&
  idempleado=<?php echo $Especialidades["Intendencia_ID"]; ?>&&nombreempleado=<?php echo $Especialidades["Nombre_Apellidos"]; ?>&&sucursalnombre=<?php echo $Especialidades["Nombre_Sucursal"]; ?>&&puesto=Limpieza">Capturar datos biometricos <i class="fas fa-fingerprint"></i></a>
  ...
  <a data-id="<?php echo$Especialidades["Intendencia_ID"];?>" class="btn-baja dropdown-item" >Marcar como baja <i class="fas fa-user-slash"></i></a>
</div>
<!-- Basic dropdown -->
	 </td>
...
<script>
    ...
    $(".btn-baja").click(function(){
  		id = $(this).data("id");
  		$.post("https://www.somosgrupoe.com/Administracion/RecursosHumanos/Modales/BajaIntendecia.php","id="+id,function(data){
              $("#form-edit").html(data);
              $("#Titulo").html("Historial datos de empleado");
              $("#Di").removeClass("modal-dialog modal-lg modal-notify modal-info");
              $("#Di").removeClass("modal-dialog modal-lg modal-notify modal-danger");
